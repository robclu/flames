...
/// version
/// 2. It can be configured by changing the type of the block.
...
class ResnetV2Impl : public torch::nn::Module {
  ...
  static constexpr int64_t fc_inputs          = 2048;
 ...
 public:
  ...
  : _classes{classes},
    ...
    _fc{fc_inputs, classes} {
    make_layer(_layer_1, layer_1_out_channels, layer_sizes[0], stride_2);
    make_layer(_layer_2, layer_2_out_channels, layer_sizes[1], stride_2);
    make_layer(_layer_3, layer_3_out_channels, layer_sizes[2], stride_2);
    make_layer(_layer_4, layer_4_out_channels, layer_sizes[3]);
 ...
 private:
  ...
  /// the layer, and \p stride for the convolutions in the layer.
  ...
  auto make_layer(
    Layer& layer, int64_t out_channels, int64_t blocks, int64_t stride = 1)
    -> void {
...
/// We can't use the macro here because of the template.
/// \tparam Block The type of the block for the network.
template <typename Block>
